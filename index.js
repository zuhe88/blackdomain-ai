...
const S = {
  ...
  mode: {}, pendingMoney: {}, pendingRoom: {}, flow: {}, tianmen: {}, slot: {}, wc: {}, mlb: {}, cache539: {},
};
...
const quickSports = () => q([["世足"], ["MLB"]]);
const quickWorldCup = () => q([["賽程查詢"], ["球隊查詢"], ["AI精選"], ["冠軍預測"]]);
const quickMLB = () => q([["今日賽程"], ["AI精選"]]);
...
function gen539(mode) {
  ...
  const nums = Array.from({ length: 39 }, (_, i) => i + 1).sort(() => Math.random() - 0.5).slice(0, 5).sort((a, b) => a - b).map((n) => String(n).padStart(2, "0"));
  S.cache539[key] = nums;
  return nums;
}
...
async function fetchMlbGames(offset = 0) {
  ...
  const games = (data.events || []).map((e) => {
    ...
    const home = c.find((x) => x.homeAway === "home")?.team?.displayName || "Home";
    const away = c.find((x) => x.homeAway === "away")?.team?.displayName || "Away";
    return { id: e.id, home: teamZh(home), away: teamZh(away), time: new Date(e.date).toLocaleString("zh-TW", { timeZone: "Asia/Taipei", hour12: false }), rawHome: home, rawAway: away };
  });
...
async function handleEvent(event) {
  ...
  const needsVip = ["百家樂", "電子", "電子AI", "539", "539AI", "539 AI", "莊", "閒", "和", "體育", "世足", "MLB", "AI配注", "天門五關", "自由配注", "DG", "MT"].includes(text)
    || /^mt/i.test(text) || /^dg/i.test(text) || (/^\d{1,6}$/.test(text) && ["awaitMoney", "selectGame", "custom", "mlbSelect"].includes(S.flow[uid] || S.wc[uid]?.mode || S.slot[uid]?.mode || S.mlb[uid]?.mode));
  ...
  if (text === "百家樂") { clearSessions(uid); S.flow[uid] = "awaitPlatform"; return client.replyMessage(event.replyToken, { type: "text", text: `━━━━━━━━━━\n⚡ 黑域AI已啟動\n━━━━━━━━━━\n\n請選擇平台：\n\n• DG\n• MT`, quickReply: q([["DG"], ["MT"]]) }); }
  if ((lower === "dg" || lower === "mt") && S.flow[uid] === "awaitPlatform") { S.flow[uid] = "awaitRoom"; return client.replyMessage(event.replyToken, { type: "text", text: `━━━━━━━━━━\n🤖 黑域AI已啟動\n━━━━━━━━━━\n\n請輸入房間號碼\n\n範例：\nDG RB01\nMT 01` }); }
  const validMT = /^mt\s*(?:0?[1-9]|1[0-3]|3a|13a)$/i.test(text), validDG = /^dg\s*(?:0?[1-7]|rb\s*0?[1-7]|s\s*0?[1-7])$/i.test(text);
  if (validMT || validDG) { S.pendingRoom[uid] = roomName(text); S.flow[uid] = "awaitMoney"; S.baccarat[uid] = []; S.result[uid] = []; S.lastPred[uid] = null; S.lastBet[uid] = null; return client.replyMessage(event.replyToken, { type: "text", text: `━━━━━━━━━━\n🤖 黑域AI數據同步成功\n━━━━━━━━━━\n\n同步房間：\n${S.pendingRoom[uid]}\n\n✓ 房間同步完成\n✓ 牌路資料載入\n✓ 模型運算啟動\n\n━━━━━━━━━━\n\n請輸入本金：\n\n例如：\n1000\n5000\n10000` }); }
  if (/^mt/i.test(text) || /^dg/i.test(text)) return client.replyMessage(event.replyToken, { type: "text", text: "查無此房間" });
  if (/^\d+$/.test(text) && S.flow[uid] === "awaitMoney") { const money = Number(text); if (money < 100) return client.replyMessage(event.replyToken, { type: "text", text: "本金金額過低，請重新輸入。" }); resetMoney(uid, money); S.pendingMoney[uid] = money; S.flow[uid] = "awaitMode"; return client.replyMessage(event.replyToken, { type: "text", text: `━━━━━━━━━━\n💰 黑域AI資金配置\n━━━━━━━━━━\n\n目前本金：\n${money}\n\n請選擇模式：\n\n1️⃣ AI配注\n2️⃣ 天門五關\n3️⃣ 自由配注\n\n━━━━━━━━━━`, quickReply: quickMoney() }); }
  ...
  if (text === "天門五關" || (text === "2" && S.flow[uid] === "awaitMode")) { const money = S.pendingMoney[uid] || 1000, plan = makeTianmen(money); resetMoney(uid, money); S.mode[uid] = "tianmen"; S.flow[uid] = "playing"; S.tianmen[uid] = { level: 1, ...plan }; if (money < 1000) return client.replyMessage(event.replyToken, { type: "text", text: `━━━━━━━━━━\n⛩️ 黑域AI天門配置\n━━━━━━━━━━\n\n目前本金：\n${money}\n\n⚠️ 不建議使用天門模式\n\n建議本金至少：\n1000以上\n\n目前較適合：\nAI配注模式\n\n━━━━━━━━━━`, quickReply: quickMoney() }); return client.replyMessage(event.replyToken, { type: "text", text: startAnalyze(uid), quickReply: quickBaccarat() }); }
  ...
  if (["莊", "閒", "和"].includes(text)) { applyResult(uid, text); S.baccarat[uid].push(text); if (S.baccarat[uid].length > 20) S.baccarat[uid].shift(); const pred = baccaratPick(S.baccarat[uid]), bet = currentBet(uid); S.lastPred[uid] = pred; S.lastBet[uid] = bet; const extra = [baccaratWarning(S.baccarat[uid]), baccaratSpecial(S.baccarat[uid])].filter(Boolean).map((x) => `\n\n${x}`).join(""); return client.replyMessage(event.replyToken, { type: "text", text: baccaratReply(uid, pred, bet, extra), quickReply: quickBaccarat() }); }
  ...
  if (mode539) { const nums = gen539(mode539), date = twDate().slash; return client.replyMessage(event.replyToken, { type: "text", text: `━━━━━━━━━━\n📊 ${text}\n━━━━━━━━━━\n\n預測日期：\n${date}\n\nAI建議號碼：\n\n${nums.join("　")}\n\n主推號：\n${nums[0]} / ${nums[2]}\n\n⚠️ 僅供娛樂分析參考`, quickReply: quick539(text) }); }

  if (text === "體育") { clearSessions(uid); return client.replyMessage(event.replyToken, { type: "text", text: `━━━━━━━━━━\n🏆 黑域體育AI\n━━━━━━━━━━\n\n請選擇項目：\n\n• 世足\n• MLB\n\n━━━━━━━━━━`, quickReply: quickSports() }); }
  if (text === "MLB") { clearSessions(uid, "mlb"); return client.replyMessage(event.replyToken, { type: "text", text: `━━━━━━━━━━\n⚾ 黑域MLB AI\n━━━━━━━━━━\n\n請選擇功能：\n\n• 今日賽程\n• AI精選\n\n━━━━━━━━━━`, quickReply: quickMLB() }); }
  if (text === "今日賽程") { let data = await fetchMlbGames(0); if (!data.games.length) data = await fetchMlbGames(1); S.mlb[uid] = { mode: "mlbSelect", games: data.games }; const msg = data.games.map((g, i) => `${i + 1}️⃣ ${g.away} vs ${g.home}\n🕒 ${g.time}`).join("\n\n"); return client.replyMessage(event.replyToken, { type: "text", text: `━━━━━━━━━━\n⚾ MLB賽程\n━━━━━━━━━━\n\n${msg || "目前查無賽程"}\n\n━━━━━━━━━━\n請選擇場次查看AI分析`, quickReply: q(data.games.map((_, i) => [`${i + 1}`])) }); }
  if (text === "AI精選" && S.mlb[uid]?.games?.length) { const g = pick(S.mlb[uid].games); return client.replyMessage(event.replyToken, { type: "text", text: mlbAnalyze(g), quickReply: quickMLB() }); }
  if (/^\d+$/.test(text) && S.mlb[uid]?.mode === "mlbSelect") { const g = S.mlb[uid].games[Number(text) - 1]; if (!g) return client.replyMessage(event.replyToken, { type: "text", text: "查無此場次" }); return client.replyMessage(event.replyToken, { type: "text", text: mlbAnalyze(g), quickReply: quickMLB() }); }
