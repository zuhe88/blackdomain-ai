...
const S = {
  ...
  mode: {}, pendingMoney: {}, pendingRoom: {}, betLimit: {}, flow: {}, tianmen: {},
  slot: {}, sport: {}, wc: {}, mlb: {}, nba: {}, cache539: {},
};

function pick(a) { return a[Math.floor(Math.random() * a.length)]; }
function q(items) {
  return { items: items.map(([label, text = label]) => ({ type: "action", action: { type: "message", label, text } })) };
}
...
function quickWorldCup() { return q([["賽程查詢", "世足賽程查詢"], ["球隊查詢", "世足球隊查詢"], ["AI精選", "世足AI精選"], ["冠軍預測", "世足冠軍預測"]]); }
function quickMLB() { return q([["近日賽程", "MLB近日賽程"], ["AI精選", "MLB AI精選"]]); }
function quickNBA() { return q([["近日賽程", "NBA近日賽程"], ["AI精選", "NBA AI精選"]]); }
...
function tw539Date() {
  const now = new Date(new Date().toLocaleString("en-US", { timeZone: "Asia/Taipei" }));
  if (now.getHours() > 20 || (now.getHours() === 20 && now.getMinutes() >= 30)) now.setDate(now.getDate() + 1);
...
function twTime(ts) { return new Date(Number(ts)).toLocaleString("zh-TW", { timeZone: "Asia/Taipei" }); }
function noVip() { return `━━━━━━━━━━
🔐 黑域AI權限尚未開通
━━━━━━━━━━
...
📲 開通請私訊
LINE：zu88.8`; }
async function getVip(uid) { const { data } = await supabase.from("vip_users").select("*").eq("user_id", uid).maybeSingle(); return data; }
async function isVip(uid) { const data = await getVip(uid); return !!data && Number(data.expire_time) > Date.now(); }
...
function resetMoney(uid, money) {
  S.startBankroll[uid] = money; S.bankroll[uid] = money; S.result[uid] = [];
  S.lastPred[uid] = null; S.lastBet[uid] = null; S.mode[uid] = null; S.tianmen[uid] = null;
}
function profit(uid) { return (S.bankroll[uid] || 0) - (S.startBankroll[uid] || 0); }
function betUnit(bankroll) { if (bankroll >= 500000) return 10000; if (bankroll >= 10000) return 1000; return 100; }
function roundBet(amount, bankroll) { const unit = betUnit(bankroll); return Math.max(unit, Math.floor(amount / unit) * unit); }
function aiBet(uid) {
  const b = S.bankroll[uid] || S.startBankroll[uid] || 1000;
  const limit = S.betLimit[uid] || b;
  const p = profit(uid);
  let minRate = 0.08, maxRate = 0.18;
  if (b >= 10000) { minRate = 0.12; maxRate = 0.35; }
  if (b >= 100000) { minRate = 0.08; maxRate = 0.28; }
  if (p > 0) maxRate += 0.03;
  const style = Math.random();
  let rate = style < 0.25 ? minRate : style < 0.75 ? minRate + Math.random() * (maxRate - minRate) : maxRate;
  return Math.max(betUnit(b), roundBet(Math.min(b * rate, limit), b));
}
function makeTianmen(money) { const base = Math.max(100, Math.floor(money / 60 / 100) * 100); const levels = [1, 3, 7, 15, 31].map((x) => base * x); return { base, levels, total: levels.reduce((a, b) => a + b, 0) }; }
...
function startAnalyze(uid) { const pred = pick(["莊", "閒"]); const bet = currentBet(uid); S.lastPred[uid] = pred; S.lastBet[uid] = bet; S.flow[uid] = "playing"; return baccaratReply(uid, pred, bet); }

function slotAnalysis(game, room) { const n = Number(room); const score = (n * 7) % 100; if (score >= 70) return { game, room: n, status: "高波動區", suggestion: "可進場", reason: "倍率區同步完成" }; if (score >= 45) return { game, room: n, status: "數據偏強", suggestion: "小注試水", reason: "倍率波動偏強" }; return { game, room: n, status: "回吐區", suggestion: "建議觀望", reason: "目前回吐波動偏高" }; }
function slotText(a) { return `━━━━━━━━━━
⚡ 黑域電子AI同步完成
━━━━━━━━━━
...
⚠️ 僅供娛樂分析參考`; }
function gen539(mode) { const key = `${tw539Date()}-${mode}`; if (S.cache539[key]) return S.cache539[key]; const nums = Array.from({ length: 39 }, (_, i) => i + 1).sort(() => Math.random() - 0.5).slice(0, 5).sort((a, b) => a - b).map((n) => String(n).padStart(2, "0")); S.cache539[key] = nums; return nums; }
...
function wcTeamSearch(team) {
  const results = [];
  Object.entries(worldCupSchedule || {}).forEach(([date, games]) => {
    games.forEach((g) => {
      const home = g.home || "";
      const away = g.away || "";
      if (home.includes(team) || away.includes(team)) results.push({ date, ...g });
    });
  });
  return results;
}
...
const mlbName = {
  "Arizona Diamondbacks": "響尾蛇", "Atlanta Braves": "勇士", "Baltimore Orioles": "金鶯", "Boston Red Sox": "紅襪", "Chicago Cubs": "小熊", "Chicago White Sox": "白襪", "Cincinnati Reds": "紅人", "Cleveland Guardians": "守護者", "Colorado Rockies": "洛磯", "Detroit Tigers": "老虎", "Houston Astros": "太空人", "Kansas City Royals": "皇家", "Los Angeles Angels": "天使", "Los Angeles Dodgers": "道奇", "Miami Marlins": "馬林魚", "Milwaukee Brewers": "釀酒人", "Minnesota Twins": "雙城", "New York Mets": "大都會", "New York Yankees": "洋基", "Oakland Athletics": "運動家", "Athletics": "運動家", "Philadelphia Phillies": "費城人", "Pittsburgh Pirates": "海盜", "San Diego Padres": "教士", "San Francisco Giants": "巨人", "Seattle Mariners": "水手", "St. Louis Cardinals": "紅雀", "Tampa Bay Rays": "光芒", "Texas Rangers": "遊騎兵", "Toronto Blue Jays": "藍鳥", "Washington Nationals": "國民",
};
const mlbPower = { 道奇: 92, 洋基: 90, 勇士: 88, 費城人: 86, 太空人: 85, 水手: 82, 教士: 82, 藍鳥: 80, 紅襪: 79, 大都會: 78, 小熊: 77, 守護者: 76, 老虎: 75, 雙城: 74, 紅雀: 73, 光芒: 72, 遊騎兵: 72, 金鶯: 72, 紅人: 71, 皇家: 70, 釀酒人: 70, 巨人: 69, 天使: 68, 海盜: 67, 國民: 66, 馬林魚: 65, 響尾蛇: 65, 洛磯: 62, 白襪: 60, 運動家: 60 };
const nbaPower = { 尼克: 88, 騎士: 84, 雷霆: 90, 馬刺: 86, 湖人: 82, 灰狼: 83, 塞爾提克: 89, 金塊: 87, 勇士: 81, 獨行俠: 80 };
function teamZh(name) { return mlbName[name] || name; }
...
function mlbAnalyze(g) {
  const hp = mlbPower[g.home] || 70, ap = mlbPower[g.away] || 70;
  const homeScore = hp + 3 + Math.random() * 8, awayScore = ap + Math.random() * 8;
...
function nbaAnalyze(g) {
  const hp = nbaPower[g.home] || 78, ap = nbaPower[g.away] || 78;
  const homeScore = hp + 3 + Math.random() * 8, awayScore = ap + Math.random() * 8;
  const side = homeScore >= awayScore ? g.home : g.away;
  const diff = Math.abs(homeScore - awayScore);
  const risk = diff >= 10 ? "中低風險" : diff >= 5 ? "中風險" : "高波動";
...
async function handleEvent(event) {
  ...
  const needsVip = ["百家樂", "電子", "電子AI", "539", "539AI", "539 AI", "莊", "閒", "和", "體育", "世足", "MLB", "NBA", "AI配注", "天門五關", "自由配注", "DG", "MT", "MLB近日賽程", "MLB AI精選", "NBA近日賽程", "NBA AI精選", "世足賽程查詢", "世足球隊查詢", "世足AI精選", "世足冠軍預測"].includes(text) ||
    /^mt/i.test(text) || /^dg/i.test(text) || /^世足日期:/.test(text) || /^世足場次:/.test(text) || /^MLB場次:/.test(text) || /^NBA場次:/.test(text) ||
...
10000` });
  ...
  if (/^\d+$/.test(text) && S.flow[uid] === "awaitMoney") {
    ...
    resetMoney(uid, money); S.pendingMoney[uid] = money; S.flow[uid] = "awaitBetLimit";
    return client.replyMessage(event.replyToken, { type: "text", text: `━━━━━━━━━━
💰 黑域AI資金配置
━━━━━━━━━━
...
━━━━━━━━━━` });
  ...
  if (/^\d+$/.test(text) && S.flow[uid] === "awaitBetLimit") {
    const limit = Number(text), money = S.pendingMoney[uid];
    if (!money) { S.flow[uid] = "awaitMoney"; return client.replyMessage(event.replyToken, { type: "text", text: "請先輸入本金。" }); }
    ...
    S.betLimit[uid] = limit; S.flow[uid] = "awaitMode";
    return client.replyMessage(event.replyToken, { type: "text", text: `━━━━━━━━━━
💰 黑域AI資金配置完成
━━━━━━━━━━
...
━━━━━━━━━━`, quickReply: quickMoney() });
  ...
  if (text === "AI配注" || (text === "1" && S.flow[uid] === "awaitMode")) { S.mode[uid] = "ai"; S.flow[uid] = "playing"; S.bankroll[uid] = S.pendingMoney[uid]; S.startBankroll[uid] = S.pendingMoney[uid]; return client.replyMessage(event.replyToken, { type: "text", text: startAnalyze(uid), quickReply: quickBaccarat() }); }
...
━━━━━━━━━━`, quickReply: quickMoney() });
  ...
  if (text === "自由配注" || (text === "3" && S.flow[uid] === "awaitMode")) { S.mode[uid] = "free"; S.flow[uid] = "playing"; return client.replyMessage(event.replyToken, { type: "text", text: startAnalyze(uid), quickReply: quickBaccarat() }); }
  if (text === "重新設定本金") { resetMoney(uid, 0); S.flow[uid] = "awaitMoney"; return client.replyMessage(event.replyToken, { type: "text", text: `━━━━━━━━━━
💰 黑域AI資金重置
━━━━━━━━━━
...
10000` }); }
  if (["莊", "閒", "和"].includes(text)) {
    ...
    if (S.mode[uid] !== "free" && (S.bankroll[uid] || 0) <= 0) { S.bankroll[uid] = 0; return client.replyMessage(event.replyToken, { type: "text", text: `━━━━━━━━━━
⚠️ 黑域AI已停止分析
━━━━━━━━━━
...
━━━━━━━━━━`, quickReply: q([["重新設定本金"]]) }); }
    S.baccarat[uid].push(text); if (S.baccarat[uid].length > 20) S.baccarat[uid].shift();
...
• 539冷號`, quickReply: quick539() });
  ...
  if (mode539) { const nums = gen539(mode539), date = tw539Date(); return client.replyMessage(event.replyToken, { type: "text", text: `━━━━━━━━━━
📊 ${text}
━━━━━━━━━━
...
⚠️ 僅供娛樂分析參考`, quickReply: quick539(text) }); }
  ...
  if (text === "體育") { clearSessions(uid); S.sport[uid] = "sports"; return client.replyMessage(event.replyToken, { type: "text", text: `━━━━━━━━━━
🏆 黑域體育AI
━━━━━━━━━━
...
━━━━━━━━━━`, quickReply: quickSports() }); }
  if (text === "MLB") { clearSessions(uid, "mlb"); S.sport[uid] = "mlb"; S.mlb[uid] = { mode: "menu", games: [] }; return client.replyMessage(event.replyToken, { type: "text", text: `━━━━━━━━━━
⚾ 黑域MLB AI
━━━━━━━━━━
...
━━━━━━━━━━`, quickReply: quickMLB() }); }
  if (text === "NBA") { clearSessions(uid, "nba"); S.sport[uid] = "nba"; S.nba[uid] = { mode: "nbaSelect", games: [{ away: "騎士", home: "尼克", time: "2026/5/22 08:00:00" }, { away: "雷霆", home: "馬刺", time: "2026/5/23 08:30:00" }] }; return client.replyMessage(event.replyToken, { type: "text", text: `━━━━━━━━━━
🏀 黑域NBA AI
━━━━━━━━━━
...
━━━━━━━━━━`, quickReply: quickNBA() }); }
  ...
  if (text === "MLB近日賽程") {
    S.sport[uid] = "mlb"; let games = [];
    try { for (let i = -1; i < 7; i++) { const data = await fetchMlbGames(i); if (data.games?.length) { games = data.games; break; } } }
    catch (err) { console.log("MLB API ERROR:", err.message); return client.replyMessage(event.replyToken, { type: "text", text: "MLB賽程資料暫時無法同步，請稍後再試。", quickReply: quickMLB() }); }
    if (!games.length) return client.replyMessage(event.replyToken, { type: "text", text: "目前查無MLB近日賽程。", quickReply: quickMLB() });
    const showGames = games.slice(0, 10); S.mlb[uid] = { mode: "mlbSelect", games: showGames };
...
━━━━━━━━━━
請選擇場次查看AI分析`, quickReply: q(showGames.map((_, i) => [`${i + 1}`, `MLB場次:${i + 1}`])) });
  }
  if (text === "MLB AI精選") {
    S.sport[uid] = "mlb"; let games = S.mlb[uid]?.games || [];
    if (!games.length) { for (let i = -1; i < 7; i++) { const data = await fetchMlbGames(i); if (data.games?.length) { games = data.games.slice(0, 10); break; } } S.mlb[uid] = { mode: "mlbSelect", games }; }
  ...
  if (/^MLB場次:\d+$/.test(text)) { const n = Number(text.split(":")[1]); const g = S.mlb[uid]?.games?.[n - 1]; if (!g) return client.replyMessage(event.replyToken, { type: "text", text: "查無此場次", quickReply: quickMLB() }); return client.replyMessage(event.replyToken, { type: "text", text: mlbAnalyze(g), quickReply: quickMLB() }); }

  if (text === "NBA近日賽程") { S.sport[uid] = "nba"; const games = S.nba[uid]?.games || []; const msg = games.map((g, i) => `${i + 1}️⃣ ${g.away} vs ${g.home}\n🕒 ${g.time}`).join("\n\n"); return client.replyMessage(event.replyToken, { type: "text", text: `━━━━━━━━━━
🏀 NBA近日賽程（台灣時間）
━━━━━━━━━━
...
━━━━━━━━━━
請選擇場次查看AI分析`, quickReply: q(games.map((_, i) => [`${i + 1}`, `NBA場次:${i + 1}`])) }); }
  if (text === "NBA AI精選") { S.sport[uid] = "nba"; const games = S.nba[uid]?.games || []; if (!games.length) return client.replyMessage(event.replyToken, { type: "text", text: "目前查無NBA賽程，請稍後再試。", quickReply: quickNBA() }); return client.replyMessage(event.replyToken, { type: "text", text: nbaAnalyze(pick(games)), quickReply: quickNBA() }); }
  if (/^NBA場次:\d+$/.test(text)) { const n = Number(text.split(":")[1]); const g = S.nba[uid]?.games?.[n - 1]; if (!g) return client.replyMessage(event.replyToken, { type: "text", text: "查無此場次", quickReply: quickNBA() }); return client.replyMessage(event.replyToken, { type: "text", text: nbaAnalyze(g), quickReply: quickNBA() }); }

  if (text === "世足") { clearSessions(uid, "wc"); S.sport[uid] = "wc"; S.wc[uid] = { mode: "menu", page: 0, games: [] }; return client.replyMessage(event.replyToken, { type: "text", text: `━━━━━━━━━━
⚽ 黑域世足系統
━━━━━━━━━━
...
━━━━━━━━━━`, quickReply: quickWorldCup() }); }
  if (text === "世足賽程查詢") { S.sport[uid] = "wc"; S.wc[uid] = { mode: "date", page: 0, games: [] }; return client.replyMessage(event.replyToken, { type: "text", text: `━━━━━━━━━━
📅 世足賽程查詢
🕒 全部為台灣時間
━━━━━━━━━━

請選擇日期：`, quickReply: wcDates(0) }); }
  if (["世足日期下一頁", "世足日期上一頁"].includes(text) && S.sport[uid] === "wc") { if (!S.wc[uid]) S.wc[uid] = { mode: "date", page: 0, games: [] }; S.wc[uid].mode = "date"; S.wc[uid].page = text === "世足日期下一頁" ? (S.wc[uid].page || 0) + 1 : Math.max(0, (S.wc[uid].page || 0) - 1); return client.replyMessage(event.replyToken, { type: "text", text: `━━━━━━━━━━
📅 世足賽程查詢
🕒 全部為台灣時間
━━━━━━━━━━
...
賽前資料完整後開放。`, quickReply: quickWorldCup() });
  if (text === "世足冠軍預測") return client.replyMessage(event.replyToken, { type: "text", text: `━━━━━━━━━━
🏆 黑域AI冠軍預測
━━━━━━━━━━
